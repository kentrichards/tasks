...
describe('deleting lists', () => {
  ...
  test('deleting a list deletes its tasks', async () => {
    ...
    const listWithTask = await List.findOne({
      name: helper.initialLists[0].name
    })
    ...
    expect(tasksAtEnd).toHaveLength(
      helper.initialTasks.length - numberOfTasksToBeDeleted
    )
  })
})
...
describe('updating a list', () => {
  ...
  test('user field cannot be updated', async () => {
    ...
    expect(JSON.stringify(updateResponse.body.user)).toEqual(
      JSON.stringify(listToUpdate.user)
    )
  })
